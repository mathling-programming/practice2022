...
class AltParser(Parser):
    ...
    def __call__(self, tokens):
        ...
        # for i in self.par:
        #     yield from i(tokens)
        yield from chain(self.p1(tokens), self.p2(tokens))
...
class OptionalParser(Parser):
    ...
    def __call__(self, tokens):
        if [i for i in self.p(tokens)]:
            return self.p(tokens)
        else:
...
class StarParser(Parser):
    ...
    детьми составляющей результата должны быть составляющие, возвращаемые нижележащим парсером'''
    ...
    def __call__(self, tokens):
        ...
        yield (Constituent(tag=None, children=tuple(self.children1), words=tuple(self.words1)),
               [w for w in tokens if w not in self.words1])

print('pr')
for i in AltParser(WordParser('a'), WordParser('b'))(['a', 'b', 'b']):
    print(i)
    print('\n')
...
# for i in StarParser(word('a'))(['a', 'b']):
#     print(i[0].words, i[0].children, i[1])

for i in StarParser(word('a') | word('b'))(['c']):
    print(i[0].words, i[0].children, i[1])
    print(type(i[0]))
print('example 3')
for i in StarParser(word('a') | word('b'))(['a', 'b', 'b']):
    print(i[0].words, i[0].children, i[1])
    print(type(i[0]))
print('jkjkj')

print('Optional example1')
print([i for i in OptionalParser(word('a'))(['a', 'b'])])
print([j for j in word('a')(['a', 'b'])])
print(OptionalParser(word('a'))(['a', 'b']) == word('a')(['a', 'b']))
print('Optional example2')
print([type(i) for i in OptionalParser(word('a'))(['b', 'c'])])

# print(j for j in i for i in (word('a') | word('b'))(['a', 'b', 'b']))
...
# for tree in S('a quick fox jump'.split()):
#     print(tree[0])